...
test('createPageSeo normalizes canonical URLs and applies social defaults', () => {
  ...
  assert.equal(seo.canonical, 'https://brandstarterkit-twojprojekt.com/blog');
  assert.equal(seo.robots, DEFAULT_ROBOTS_DIRECTIVE);
  assert.deepEqual(seo.openGraph, {
    type: 'website',
    title: 'Blog',
    description: 'Porady dla lokalnej firmy',
    url: 'https://brandstarterkit-twojprojekt.com/blog',
    siteName: SITE.displayName,
    image: 'https://brandstarterkit-twojprojekt.com/og-image.png',
  });
...
test('createPageSeo supports custom robots, article timestamps, and alternates', () => {
  const seo = createPageSeo({
    ...
    alternates: {
      ...
      'pl-PL': 'https://brandstarterkit-twojprojekt.com/polityka-prywatnosci/',
    },
  });
  ...
  assert.deepEqual(seo.alternates, {
    'en-US': 'https://brandstarterkit-twojprojekt.com/privacy-policy',
    'pl-PL': 'https://brandstarterkit-twojprojekt.com/polityka-prywatnosci',
  });
});
