...
test('buildHomeSchema links the homepage to the service business graph', () => {
  ...
  assert.equal(webPage?.['@id'], 'https://delta240mvt.com/#webpage');
  assert.deepEqual(webPage?.isPartOf, { '@id': 'https://delta240mvt.com/#website' });
...
test('buildCollectionSchema differentiates blog index and paginated archives', () => {
  const schema = buildCollectionSchema({
    canonical: 'https://delta240mvt.com/blog/2',
  ...
  assert.equal(collection?.['@id'], 'https://delta240mvt.com/blog/2#collection');
...
test('buildArticleSchema links articles to the website and author entity', () => {
  const schema = buildArticleSchema({
    title: 'Hello',
    description: 'World',
    canonical: 'https://delta240mvt.com/blog/pierwszy-wpis-startera',
  ...
  assert.equal(article?.['@id'], 'https://delta240mvt.com/blog/pierwszy-wpis-startera#article');
  assert.deepEqual(article?.author, { '@id': 'https://delta240mvt.com/#entity' });
  assert.deepEqual(article?.publisher, { '@id': 'https://delta240mvt.com/#entity' });
  assert.deepEqual(article?.mainEntityOfPage, {
    '@id': 'https://delta240mvt.com/blog/pierwszy-wpis-startera#webpage',
  });
});
...
test('buildArticleSchema accepts relative image URLs', () => {
  ...
  assert.equal(article?.image, 'https://delta240mvt.com/og-image.png');
});
...
test('buildLegalSchema exposes the Polish legal page', () => {
  ...
  assert.equal(legal?.['@id'], 'https://delta240mvt.com/polityka-prywatnosci#webpage');
